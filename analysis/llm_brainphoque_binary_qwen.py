"""Evaluate Qwen2.5-3B in-context on BrainPhoque-generated binary sequences.

This script:
  * Uses the BrainPhoque UTM implementation to *execute* randomly sampled
    BrainPhoque programs.
...
Usage (example):
  ...
  python -m neural_networks_solomonoff_induction.analysis.llm_brainphoque_binary_qwen \\
    --num_sequences 256 \\
    --sequence_length 128 \\
    --output_path qwen2_5_3b_brainphoque_binary_results.npz
...
def main() -> None:
  parser = argparse.ArgumentParser(
      description=(
          ...
          "BrainPhoque programs (in-context, no training)."
      )
  )
  parser.add_argument(
      "--num_sequences",
      type=int,
      default=256,
      help="Number of BrainPhoque-generated binary sequences to evaluate.",
  )
  parser.add_argument(
      "--sequence_length",
      type=int,
  ...
  parser.add_argument(
      ...
      type=int,
      default=0,
      help="Random seed for BrainPhoque program and sequence sampling.",
  )
  parser.add_argument(
      "--data_path",
      type=str,
  ...
  parser.add_argument(
      "--output_path",
      type=str,
      default="qwen2_5_3b_brainphoque_binary_results.npz",
  ...
  if args.data_path:
    ...
    print(
        ...
        f"{args.sequence_length} from BrainPhoque UTM..."
    )
